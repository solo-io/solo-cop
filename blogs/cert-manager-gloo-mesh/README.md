...
If you take a look at the `helm/cp-values.yaml` you can see that we have disabled the default CA cert and token generation as well as set the `glooMeshMgmtServer.relay.tlsSecret` to the `gloo-server-tls-cert` generated by cert-manager above
