...
echo "Modifying podinfo-frontend WorkspaceSettings to export to podinfo-backend Workspace"
...
echo "Modifying podinfo-backend WorkspaceSettings to import from podinfo-frontend Workspace"
kubectl apply --context ${MGMT} -f - <<EOF
apiVersion: admin.gloo.solo.io/v2
kind: WorkspaceSettings
metadata:
