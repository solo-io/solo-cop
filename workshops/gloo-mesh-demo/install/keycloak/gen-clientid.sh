...
echo "EXT endpoint: ${ENDPOINT_HTTPS_GW_CLUSTER1_EXT}"
...
curl -v -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -X PUT -H "Content-Type: application/json" -d '{"serviceAccountsEnabled": true, "authorizationServicesEnabled": true, "redirectUris": ["'https://${ENDPOINT_HTTPS_GW_CLUSTER1_EXT}'/callback","https://localhost:8443/callback"]}' $KEYCLOAK_URL/admin/realms/master/clients/${id}
