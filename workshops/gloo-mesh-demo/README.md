...
Gloo Mesh also aggregates the metrics and access logs from the different clusters. A Service Graph can then be used to monitor all the communication happening globally.
...
```
# kubectl context names of each cluster (`kubectl config get-contexts`). Update the values as necessary.
export MGMT=mgmt
