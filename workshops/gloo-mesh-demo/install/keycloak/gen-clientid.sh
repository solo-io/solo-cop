...
curl -v -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -X PUT -H "Content-Type: application/json" -d '{"serviceAccountsEnabled": true, "authorizationServicesEnabled": true, "redirectUris": ["'https://${1}'/callback","https://localhost:8443/callback"]}' $KEYCLOAK_URL/admin/realms/master/clients/${id}
