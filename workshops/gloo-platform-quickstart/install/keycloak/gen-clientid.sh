...
# Create first user
curl -v -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -X POST -H "Content-Type: application/json" -d '{"username": "gloo-gateway", "email": "gloo-gateway@solo.io", "enabled": true, "attributes": {"group": "users"}, "credentials": [{"type": "password", "value": "solo.io", "temporary": false}]}' $INTERNAL_KEYCLOAK_URL/admin/realms/master/users
